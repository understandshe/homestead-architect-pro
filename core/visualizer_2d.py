"""
Professional 2D Site-Plan Visualizer — Premium v3
...
def _bezier_path(ax, points, width=10, color='#E8DCC8', alpha=0.92, zorder=4):
    ...
    # Build smooth curve via catmull-rom → bezier
...
# ─────────────────────────────────────────────────────────────────────────────
#  Single tree (top-down aerial view — blob canopy)
# ─────────────────────────────────────────────────────────────────────────────
TREE_COLORS = {
...
class Visualizer2D:
    ...
    # ── Perimeter border (dense shrubs like in reference) ─────────────────────
    ...
    def _curved_paths(self, ax, layout, L, W):
        """
        Curved bezier gravel paths:
        - Main curved entrance from south boundary → house door
        - Cross path curving E-W at house level
        - Individual spur to each feature (curves away from obstacles)
        All registered in collision registry.
        """
        hx,hy,hw,hh = self._house_bbox(layout, L, W)
        door_cx = hx+hw/2
        door_cy = hy
        cross_y = hy+hh*0.5
        main_w  = max(12, min(L,W)*0.028)
        sec_w   = max(8,  min(L,W)*0.018)
        spur_w  = max(6,  min(L,W)*0.014)
        gravel  = '#E8DCC8'
        gravel2 = '#D7CCC8'

        # Register main paths in collision registry
        # Main entrance (south boundary → house door)
        self._reg.add_rect(door_cx-main_w/2, 0, main_w, hy)

        # Main entrance: gentle S-curve
        _bezier_path(ax, [
            (door_cx, 0),
            (door_cx + L*0.03, hy*0.3),
            (door_cx - L*0.02, hy*0.6),
            (door_cx, door_cy),
        ], width=main_w, color=gravel, zorder=4)

        # E-W cross path (gently curved)
        self._reg.add_rect(0, cross_y-sec_w/2, L, sec_w)
        _bezier_path(ax, [
            (0, cross_y),
            (L*0.25, cross_y + W*0.015),
            (L*0.50, cross_y - W*0.010),
            (L*0.75, cross_y + W*0.008),
            (L,      cross_y),
        ], width=sec_w, color=gravel, zorder=4)

        # North path from house top
        _bezier_path(ax, [
            (door_cx, hy+hh),
            (door_cx + L*0.02, hy+hh + (W-hy-hh)*0.4),
            (door_cx, W),
        ], width=sec_w, color=gravel, zorder=4)

        # Spur to each feature
        features = layout.get('features',{})
        skip = {'swales','compost'}
        for key,f in features.items():
            if key in skip: continue
            if isinstance(f,list): continue
            ...
            # Bezier spur: cross_y → feature, slight curve
            mid_x = (fx+door_cx)/2 + np.random.uniform(-L*0.03, L*0.03)
            mid_y = cross_y + (fy-cross_y)*0.5 + np.random.uniform(-W*0.02, W*0.02)
            _bezier_path(ax, [
                (door_cx, cross_y),
                (mid_x, mid_y),
                (fx, fy),
            ], width=spur_w, color=gravel2, alpha=0.78, zorder=4)

    # ── House (proper aerial plan view) ───────────────────────────────────────
    ...
    def _house_plan(self, ax, layout, L, W):
        ...
        # Roof shingles — fill with grey gradient lines
        ...
        # Porch/deck (like reference image — wooden deck)
    ...
    # ── Vegetation (aerial top-down trees) ─────────────────────────────────────
